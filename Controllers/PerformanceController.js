...
const AllEmployeeAttandance = async (req, res,next) => {
  try {
    ...
    const employeeAttendance = await Promise.all(employees.map(async (employee) => {
      const { _id, firstName, avatar,email, designationLevel, employeeId, lastName, userId } = employee;
...
const EmployeeAttandanceById = async (req, res, next) => {
  try {
    ...
    const { _id:employeeDocId, firstName, avatar, designationLevel, employeeId, lastName } = employee;
    const {calculatedAttendance} = attandance ? await calculateTotalWorkingHours(attandance) : {calculatedAttendance:null};
    

    return res.status(200).json({ success: true, Data: {...calculatedAttendance,employeeDocId, firstName, avatar, designationLevel, employeeId, lastName} });
